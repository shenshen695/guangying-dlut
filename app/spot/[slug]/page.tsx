...
export default function SpotDetailPage({ params }: { params: { slug: string } }) {
  ...
  return <main className="min-h-screen bg-mist"><div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10"><header className="flex items-center justify-between border-b border-ink/10 pb-5"><Link href="/" className="text-sm font-bold tracking-[.18em] text-ink">光影工大</Link><Link href="/map?route=classic-graduation" className="text-sm text-slate-500 hover:text-sea">校园摄影地图 →</Link></header>
    ...
    <section className="mt-8 flex flex-wrap gap-3"><Link href={route ? `/route/${route.slug}` : "/route/classic-graduation"} className="rounded-full bg-coral px-5 py-3 text-sm font-semibold text-white hover:bg-ink">查看工大经典毕业线 →</Link><p className="self-center text-xs text-slate-400">摄影机位待摄影师确认，路线展示不受影响。</p></section>
  </div></main>;
}
