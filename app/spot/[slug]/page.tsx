...
export default function SpotDetailPage({ params }: { params: { slug: string } }) {
  ...
  return <main className="min-h-screen bg-mist"><div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 lg:px-12 lg:py-10">
    <header className="flex items-center justify-between border-b border-ink/10 pb-5"><Link href="/" className="text-sm font-bold tracking-[.18em] text-ink">光影大工</Link><Link href="/map?route=campus-highlights" className="text-sm text-slate-500 hover:text-sea">Q版校园摄影地图 →</Link></header>
    <section className="pb-10 pt-14 sm:pt-20"><p className="text-[11px] font-semibold tracking-[.2em] text-sea">SPOT / {spot.area}</p><div className="mt-4 flex flex-wrap items-end justify-between gap-4"><h1 className="text-4xl font-semibold tracking-tight text-ink sm:text-6xl">{spot.name}</h1><span className="rounded-full border border-coral/35 bg-[#fffaf7] px-3 py-1.5 text-xs text-coral">{spot.featured ? "校园推荐地标" : hasPhotos ? `${spot.images!.length} 张机位样片` : "样片待补充"}</span></div><p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">{spot.description}</p>{spot.seasonNote && <p className="mt-3 inline-flex rounded-full bg-[#fff1eb] px-4 py-2 text-sm font-semibold text-coral">推荐：{spot.seasonNote}</p>}</section>
