...
function getSupabaseClient() {
  ...
  if (!supabaseUrl || !supabaseAnonKey || supabaseUrl.includes("你的") || supabaseAnonKey.includes("你的")) {
    return null;
  }
...
export async function getPreferences(userId: string): Promise<UserPreferences | null> {
  ...
  if (error) {
    console.warn("Failed to read user preferences:", error.message);
    return null;
  }
...
export async function upsertPreferences(update: PreferenceUpdate): Promise<void> {
  ...
  if (error) {
    console.warn("Failed to upsert user preferences:", error.message);
  }
}
...
export async function updateDisliked(userId: string, styles: GraduationStyle[]): Promise<GraduationStyle[]> {
  ...
  if (error) {
    console.warn("Failed to update disliked styles:", error.message);
  }
...
export async function deletePreferences(userId: string): Promise<boolean> {
  ...
  if (error) {
    console.warn("Failed to delete user preferences:", error.message);
    return false;
  }
