...
export default function SpotSubmissionClient({ spot }: { spot: Spot }) {
  ...
  return <main className="min-h-screen bg-mist pb-12">
    ...
    <div className="mx-auto max-w-4xl px-5 pt-7 sm:px-8 sm:pt-10">
      {step !== "success" && <><div className="flex items-center gap-2 text-[11px] font-semibold tracking-[.16em] text-sea"><span>PHOTOGRAPHER CONTRIBUTION</span><span className="rounded-full bg-coral/15 px-2 py-1 text-[10px] tracking-normal text-coral">本地 Demo</span></div><h1 className="mt-3 text-3xl font-semibold tracking-tight text-ink sm:text-4xl">把你的毕业照变成一份拍摄攻略</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">上传作品，AI 会帮你整理机位、光线和拍摄建议；最后由你确认，分享给下一位来到{spot.name}的人。</p><nav className="mt-7 flex gap-1 overflow-x-auto pb-1" aria-label="投稿步骤">{steps.map((item, index) => <div key={item.id} className="flex shrink-0 items-center gap-1"><span className={`rounded-full px-3 py-2 text-xs font-semibold ${step === item.id ? "bg-ink text-white" : steps.findIndex((candidate) => candidate.id === step) > index ? "bg-sea/15 text-sea" : "bg-white text-slate-400"}`}>{index + 1}. {item.label}</span>{index < steps.length - 1 && <span className="h-px w-4 bg-ink/15" />}</div>)}</nav></>}
